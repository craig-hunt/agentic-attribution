...
**Code a team inherits gets changed.** The value of a starting point depends on whether the next engineer can modify it confidently. Tests that catch regressions are the mechanism that makes inherited code safe to touch.

**Coverage measures the wrong thing.** Line coverage reports whether a line executed during a test run. It says nothing about whether any assertion would fail if that line were wrong. A test suite can reach ninety percent coverage while asserting almost nothing, and that suite provides false confidence more dangerous than no suite at all.
...
Money moves through this system. A settlement that credits the wrong publisher, or credits nobody, is the failure mode that matters most, and it sits precisely at the seam where the three languages meet.

## Decision

**Three testing tiers, each answering a question the others cannot.**
...
The threshold applies to the logic that matters. Generated code, plain data structures, and framework glue are excluded through configuration rather than by lowering the bar globally.

### Tier 3: Cypress end-to-end regression
...
cypress/
├── constants/
│   ├── selectors.ts      data-cy attribute names
...
**Selectors bind to `data-cy` attributes exclusively.** Never CSS classes, never element hierarchy, never text content. Class names change during styling work and hierarchy changes during refactors, and either one produces a failing suite that reports nothing about correctness.
...
- Mutation testing converts "we have tests" into "our tests would catch a defect," which are different claims and only one of them is worth making.
- The Cypress suite validates the three-language contract that unit tests structurally cannot reach. When the Go encoder and TypeScript decoder drift, the E2E suite fails and the unit suites stay green, which is exactly the signal needed.
...
- A team inheriting this can refactor with real confidence, which is the entire point of shipping a starting point rather than a sketch.
...
- The 70% threshold is a judgment, not a law. It sits high enough to force meaningful assertions and low enough to avoid chasing mutants in code where the effort earns nothing.
- Cypress testnet dependency can be stubbed at the facilitator boundary for deterministic runs, with a separate suite exercising live settlement. That tradeoff gets decided during implementation.

## Alternatives considered

**Unit tests with a coverage threshold, no mutation testing.** The standard approach, and far cheaper in CI time. Rejected because coverage measures execution rather than assertion. A suite can execute every line while asserting almost nothing, and that produces confidence unbacked by evidence. Mutation testing is the only widely available technique that measures whether the tests would actually catch a defect.

**No tests, on the grounds that this is a demonstration.** Common for demo repositories and defensible when the demo exists only to be looked at. Rejected because this demo exists to be productionalized. Handing a team untested code and calling it a starting point transfers all the verification burden to them while claiming to have reduced it.

**Integration tests instead of unit tests.** Fewer tests, closer to real behavior, less coupling to implementation detail. Rejected as a replacement because integration tests localize failures poorly. When a commission calculation is wrong, a unit test names the function and an integration test names the request. Both tiers stay, serving different purposes.
