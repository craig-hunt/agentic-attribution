...
The choice matters because merchant integration is where affiliate platforms spend enormous engineering effort in production. Real integrations involve catalog synchronization, order lifecycle management, inventory reconciliation, fulfillment status, returns handling, and commercial negotiation. None of that engineering answers the question this project exists to answer.
...
- The merchant's verification path is real. It performs actual Ed25519 signature verification and actual facilitator calls, so the demo proves the mechanism works against an independent counterparty rather than against itself.
- A team productionalizing this can replace the simulated merchant with a real integration without touching the attribution layer, because the boundary is an HTTP contract rather than shared code.
- The demo runs entirely self-contained. No partner dependency, no credentials to obtain, no third-party availability risk.

**Negative.**

- The demo does not prove real merchants would adopt this. Merchant cooperation is a commercial question, and no technical demonstration resolves it.
...
- A skeptical reviewer could reasonably note that a merchant written by the same author will always verify assertions correctly. That criticism holds, and the mitigation is that the verification code implements a published, independently checkable signature scheme rather than a private handshake.
