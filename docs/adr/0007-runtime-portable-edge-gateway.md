...
ADR-0001 selected TypeScript for the edge layer and named Cloudflare Workers as the runtime. A separate decision, made later while evaluating hosting cost, established that the entire demo must run locally under Docker Compose with no hosted dependency and no account signup. Those two decisions were never reconciled.
...
The conflict surfaced a sharper question. **What does the edge layer actually depend on?** Reading back through the gateway's requirements, the answer is: `Request`, `Response`, `fetch`, and WebCrypto. Every one of those is a Web standard implemented by Workers, by Deno, by Bun, by Node 18 and later, and by browsers. Nothing in the gateway needs a Cloudflare-specific API.
...
- **`src/node.ts`** adapts `node:http` to the handler for local Docker. This is what `docker compose up` runs.
...
- One verification implementation serves Node, Workers, and browsers. A second implementation would be a second place for the canonical signing bytes to drift away from Go.
...
- The Node adapter is code that exists solely to bridge runtimes. It stays small, roughly 40 lines, and it carries its own tests, but it remains a seam that Workers-only code would not have.
- Running under Node locally and Workers in production means the two environments differ. Behaviour that depends on runtime specifics would surface only after deployment. Restricting the gateway to Web standards is what keeps that risk small, and a lint rule enforcing no Node built-in imports keeps it small deliberately rather than by habit.
...
**Deploy a single hosted gateway that all clones share.** Removes the local gateway entirely. Rejected on two grounds. An unauthenticated endpoint calling a settlement service that writes to a database is an abuse surface with nothing in front of it, and a demo whose behaviour depends on infrastructure the reader does not control cannot be evaluated, only observed.

**Keep the real facilitator as the only path.** Strongest possible fidelity, and the on-chain transaction is genuinely compelling to watch. Rejected as a default because it puts a faucet, a wallet, and a rate limit between a reviewer and the first successful run. Retained as an opt-in, where anyone who wants that fidelity can have it.

**Mock the signature as well as the transfer.** Would remove the EIP-3009 implementation entirely and simplify the simulator considerably. Rejected because the signing path is one of the genuinely hard parts of x402 and demonstrating it is much of the point. A mock that skips signature verification would also make the mock and the real facilitator disagree about which payloads are valid, which is the worst property a test double can have.
