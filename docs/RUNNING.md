...
| OpenSearch | http://localhost:9200, or http://localhost:$OPENSEARCH_HOST_PORT when that variable is set |
...
Release the reservations from an elevated PowerShell prompt:
...
Or publish somewhere else and carry on. From bash or WSL:

```bash
OPENSEARCH_HOST_PORT=19200 make e2e-cold
```

From PowerShell, where inline assignment before a command does not work:
