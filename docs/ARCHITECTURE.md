...
Issues a single OpenSearch query containing both a `multi_match` BM25 subquery and a `knn` vector subquery, processed through a **search pipeline with a normalization processor**. Raw BM25 scores run unbounded and corpus-dependent while cosine similarity runs 0 to 1, so blending them without normalization lets BM25 dominate arbitrarily. The pipeline applies `min_max` normalization and `arithmetic_mean` combination with configurable weights.
...
**Query embedding** makes the one inference call on this path, cached aggressively on query-text hash. Ingest precomputes every product embedding.

### Attribution: minted in one place, verified in three

No standalone attribution service exists, and that absence reflects a decision rather than an omission.
...
**Replay** resists a local answer, because it requires durable state. The `consumed_assertions` table in Postgres holds it and settlement checks it inside the same transaction that records the settlement, which makes the single-use guarantee hold under concurrency. See ADR-0003.
...
**Commission split:** on confirmed settlement, the ledger records the merchant debit, the platform fee, and the publisher credit as a single atomic transaction. A settlement confirming on-chain but failing to write the ledger would create an attribution gap, so reconciliation rather than assumption connects the ledger write to the settlement confirmation.

### PHP application layer

**Responsibilities:** publisher dashboard, agent simulator UI, attribution chain visualization.

PHP 8.3 with a thin router. No framework, because the demo's application surface stays small and a framework would obscure rather than clarify. Reads through the Go services rather than hitting Postgres directly, which keeps the data contract in one place.
...
**The index never holds the system of record.** Postgres rebuilds it on demand, so a corrupted index, a mapping change, or a relevance experiment carries no data-loss risk.

**Index design:**

- Multi-field mapping: `title` as `text` for BM25 matching, `title.keyword` for exact filters and aggregations
- `knn_vector` field carrying precomputed product embeddings, HNSW algorithm, Lucene engine (see [ADR-0008](adr/0008-knn-engine-lucene.md))
...
Mappings stay largely immutable, so changing a field type requires a reindex. This pattern turns that routine rather than an outage.
...
Postgres never appears on this path. Search reads exclusively from the derived index, which explains precisely why ingest write pressure cannot degrade search latency.
...
Agent → gateway                                  signs EIP-3009, attaches assertion
         └─ verifies assertion, rejects forgeries before touching any origin
...
**This dual-path pattern carries the point.** Postgres takes the write under transactional guarantees; OpenSearch receives a derived index built for read performance. Neither compromises for the other.
...
- **Single use.** Settlement consumes each assertion ID. A replay gets rejected.
...
| OpenSearch | opensearch:2 container | Derived index, k-NN via HNSW on the Lucene engine, ONNX embeddings through ML Commons |
| Facilitator | Node container | Mock by default; set `X402_FACILITATOR_URL` for Coinbase and Base Sepolia |
| Driver | Node container | Runs a live agent population on demand; publishes no host port, reached only through the dashboard proxy. Control endpoints carry no authentication, which bounds it to a local demo. See [ADR-0010](adr/0010-live-agent-driver.md) |

**Why local rather than hosted.** Hosted free tiers do not fit this workload. Neon's free tier caps at 512MB against roughly 450MB of catalog plus staging tables, and no free hosted OpenSearch tier exists at all. Paying for either buys nothing a reviewer values, and a demo costing money to keep alive stops working the moment someone stops paying.
